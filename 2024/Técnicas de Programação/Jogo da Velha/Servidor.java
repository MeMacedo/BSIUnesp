...
public class Servidor {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            InputStreamReader jogadorXStream = new InputStreamReader(jogadorX.getInputStream());
            inX = new BufferedReader(jogadorXStream);

            // Aguarda a conexão do 2o jogador, que será o jogador O
            // E cria seus streams de entrada e saída
            jogadorO = serverSocket.accept();
            System.out.println("Jogador O conectado.");
            outO = new PrintWriter(jogadorO.getOutputStream(), true);
            InputStreamReader jogadorOStream = new InputStreamReader(jogadorO.getInputStream());
            inO = new BufferedReader(jogadorOStream);
    ...
    // Inicializa o tabuleiro com -, indicando que ainda nenhum jogador escolheu a
    // posição
        ...
        while (true) {
            ...
            if (linha != null) {
                ...
                if (verificarVitoria('X')) {
                    outX.println("Você venceu!");
                    outO.println("Você perdeu!");
                    enviarTabuleiro();
                    break;
                } else if (verificarVitoria('O')) {
                    outO.println("Você venceu!");
                    outX.println("Você perdeu!");
                    enviarTabuleiro();
                    break;
